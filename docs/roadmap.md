...
- [ ] capability 기반 정책/보호 구조로 정리
- [ ] path policy helper 중앙화 (dataset/calibration/token/log/temp)
...
### Data 강화
- [ ] 데이터셋 녹화 환경 메타데이터 표시 — 상세 페이지에서 녹화 당시 사용된 카메라 이름, arm 이름, robot type 표시. eval 때 카메라/arm 매칭이 맞는지 사전 확인 가능. (`info.json`의 features에서 image key 추출 + 추가 메타 필요 시 녹화 시점에 기록)
...
- [ ] SystemStatus 매핑 상태 경고 — 매핑 안 된 arm/camera에 명시적 경고 아이콘 표시
