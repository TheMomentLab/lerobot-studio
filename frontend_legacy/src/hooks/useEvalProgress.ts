...
export type EvalProgressStatus = 'idle' | 'starting' | 'running' | 'stopped' | 'completed' | 'error'
...
export function formatReward(value: number | null) {
  return Number.isFinite(value) ? Number(value).toFixed(4) : '--'
}

export function formatSuccess(value: number | null) {
...
export function formatElapsed(startedAtMs: number | null, endedAtMs: number | null) {
...
export function useEvalProgress({ evalLogLines, running }: UseEvalProgressArgs) {
  ...
  const beginEval = useCallback((episodes: number, currentLogCount: number) => {
  ...
  useEffect(() => {
    ...
    for (const lineItem of nextLines) {
      ...
      if (COMPLETE_MARKER.test(line)) {
        ...
        setFinalReward((prev) => (Number.isFinite(prev) ? prev : meanRewardRef.current))
        setFinalSuccess((prev) => (Number.isFinite(prev) ? prev : successRateRef.current))
      }
      ...
      if (END_MARKER.test(line)) {
        ...
        setProgressStatus((prev) => {
          ...
          if (targetEpisodesRef.current && doneEpisodesRef.current >= targetEpisodesRef.current) return 'completed'
          return 'stopped'
        })
      }
    }
  ...
  }, [evalLogLines, markError, recomputeBestWorst, updateDoneEpisodes, updateTargetEpisodes])
