...
import {
  ...
  Moon, Sun, Menu, X, ChevronRight, Terminal, ChevronUp, ChevronDown, Copy, Trash2, Play,
} from "lucide-react";
...
const NAV_GROUPS = [
  {
    ...
    label: "Hardware",
    items: [
      { path: "/", label: "System Status", icon: Monitor, badge: "ready" },
      { path: "/motor-setup", label: "Motor Setup", icon: Settings, badge: null },
      { path: "/camera-setup", label: "Camera Setup", icon: Camera, badge: "warning" },
    ],
  },
  {
    id: "operate",
    label: "Operate",
    items: [
      { path: "/teleop", label: "Teleop", icon: Play, badge: null },
      { path: "/recording", label: "Recording", icon: Video, badge: null },
    ],
  },
  {
    id: "data",
    label: "Data",
    items: [
      { path: "/dataset", label: "Dataset", icon: Database, badge: null },
    ],
  },
  {
    id: "ml",
    label: "ML",
    items: [
      { path: "/training", label: "Training", icon: Brain, badge: null },
      { path: "/evaluation", label: "Evaluation", icon: FlaskConical, badge: null },
    ],
  },
];
...
const BADGE_STYLE: Record<string, string> = {
  ...
  running: "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400",
};
                          ...
                          {item.badge && (
                            <span
                              className={cn(
                                ...
                                BADGE_STYLE[item.badge] ?? "bg-zinc-200 text-zinc-400"
                              )}
                              style={{ fontSize: "9px" }}
                            >
                              {item.badge}
      ...
      <div className="flex items-center gap-2 px-3 h-7 flex-none border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-1 ml-2">
          {PROCESS_NAMES.map((p) => (
            ...
            >
              {p === "teleop" && isRuntimeProcessRunning(procStatus, p) ? (
...
function Header({
  ...
  const { hfAuth, cycleHfAuth } = useHfAuth();
    ...
    <header className="h-11 flex-none flex items-center gap-2 px-3 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 z-50">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          title="Click to cycle HF auth state (wireframe)"
        >
          <span aria-hidden="true">🤗</span>
          <span>
            {hfAuth === "ready" ? "lerobot-user" : hfAuth === "missing_token" ? "No Token" : "Invalid"}
          </span>
        </button>
