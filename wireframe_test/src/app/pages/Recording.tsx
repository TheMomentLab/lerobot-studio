...
export function Recording() {
  ...
  const handleStart = async () => {
    ...
    const started = await apiPost<ActionResponse>("/api/record/start", {
      mode,
      totalEps,
    });
    if (!started.ok) {
  ...
  const cameraStatsRows = CAMERAS_MAPPED.map((cam) => ({
    role: cam.role,
              ...
              {recTab === "plan" && (
                <div className="flex flex-col gap-3">
                  ...
                  <FieldRow label="Dataset Repo ID">
                    <WireInput value="lerobot-user/pick_cube_dataset" placeholder="username/dataset-name" />
                  </FieldRow>
                  <FieldRow label="Task Description">
                    <WireInput placeholder="Pick the red cube and place it..." />
                  </FieldRow>
                  <div className="flex flex-col gap-2 pt-1">
                    <WireToggle label="Resume — 기존 데이터셋에 이어서 녹화" />
                    <div className="flex items-center gap-2">
                      <WireToggle
                        label="Push to Hub — 완료 후 자동 업로드"
                        checked={hfAuth === "ready"}
                        onChange={() => {}}
                      />
                      {hfAuth !== "ready" && (
                ...
                <div className="flex flex-col gap-4">
                  <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                    <div className="px-3 py-2 bg-zinc-50 dark:bg-zinc-800/30 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                      ...
                      <StatusBadge status="ready" label={`${cameraStatsRows.length}/${CAMERAS_MAPPED.length} 매핑됨`} />
                    </div>
