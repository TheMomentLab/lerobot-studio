...
function readInitialTransportMode(): TransportMode {
  const envRaw = String(import.meta.env.VITE_API_TRANSPORT_MODE ?? "mock").toLowerCase();
...
function handleGlobalWsEvents(data: unknown): void {
  ...
  if (obj.type === "api_support" && typeof obj.key === "string" && typeof obj.value === "boolean") {
    setLeStudioState((prev) => {
      const next: Record<string, boolean> = { ...prev.apiSupport, [obj.key as string]: obj.value as boolean };
...
export function subscribeTrainChannel(
  channel: "status",
  listener: (event: TrainStatusEvent) => void,
): () => void;
export function subscribeTrainChannel(
  channel: "output",
  listener: (event: TrainOutputEvent) => void,
): () => void;
export function subscribeTrainChannel(
  channel: "metric",
  listener: (event: TrainMetricEvent) => void,
): () => void;
export function subscribeTrainChannel(
  channel: TrainStreamChannel,
  listener: (event: TrainStreamEvent) => void,
): () => void {
  if (transportMode !== "passthrough") {
    return subscribeMockTrainChannel(channel, listener);
  }
